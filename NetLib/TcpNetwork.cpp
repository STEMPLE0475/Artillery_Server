...
namespace NServerNetLib
{
	...
	RecvPacketInfo TcpNetwork::DispatchPacket() // 가장 앞에 있는 패킷을 반환한다.
	{
	...
	NET_ERROR_CODE TcpNetwork::RecvSocket(const int sessionIndex)
	{
		...
		//버퍼에 이전에 읽지 않은 데이터가 남아 있는 경우 앞당김. 지연 압축 방식. (이전에 패킷을 읽을 때 미리 버퍼 압축을 하는 방식은 즉시 압축)
	...
	//버퍼 안의 패킷이랑 바디를 분리함. -> AddPacketQueue에서 처리
	NET_ERROR_CODE TcpNetwork::RecvBufferProcess(const int sessionIndex)
	{
		...
		while ((dataSize - readPos) >= PACKET_HEADER_SIZE)
		{
			...
			if (bodySize > 0)
			{
				if (bodySize > (dataSize - readPos)) // 바디까지 전부 도착했는지 확인.
				{
					readPos -= PACKET_HEADER_SIZE;
					break;
				}
